...
public class EvalSystem {
    ...
    private static final int IDEAL_UNITS = 21; //

    public EvalSystem(String program) {
    ...
    private List<Subject> loadCurriculum(String program) {
        ...
        switch (program.toLowerCase()) {
            case "bsit":
                path = "src/main/resources/org/evalsys/finalevalsystem/bsit_curriculum.xml";
                break;
            case "bsmt":
                path = "src/main/resources/org/evalsys/finalevalsystem/bsmt_curriculum.xml";
                break;
            case "bsn":
                path = "src/main/resources/org/evalsys/finalevalsystem/bsn_curriculum.xml";
                break;
            case "bsa":
                path = "src/main/resources/org/evalsys/finalevalsystem/bsa_curriculum.xml";
                break;
            default:
    ...
    // Primary recommendation method for continuing students
    public List<Subject> getRecommendedSubjects(Map<String, Boolean> academicHistory, int currentYear, int currentSemester) {
        ...
        // Add failed subjects that need to be retaken
        addRetakeSubjects(context);

        // Add major subjects for the next semester
        addNextSemesterCoreSubjects(context);

        // Add subjects from future semesters that have prerequisites satisfied
        if (context.totalUnits < MIN_UNITS) {
            addAdvancedEligibleSubjects(context);
        }

        // If still below minimum, look for any eligible subjects, even from previous semesters
        if (context.totalUnits < MIN_UNITS) {
